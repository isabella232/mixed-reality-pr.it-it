...
Quando l'app remota è pronta per il rendering del contenuto, è possibile stabilire una connessione al dispositivo Player.
...
Per stabilire una connessione dall'app remota al dispositivo Player, chiamare il ```Connect``` metodo sul contesto remoto specificando il nome host e la porta. La porta usata dal lettore di comunicazione remota olografica è **8265**.
