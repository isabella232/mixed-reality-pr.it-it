...
Per informazioni di basso livello su questo argomento, vedere l'articolo relativo al [rendering in DirectX](../native/rendering-in-directx.md) .
...
In generale, l'applicazione eseguirà il rendering di una singola fotocamera stereo. Un ciclo di rendering affidabile supporterà più fotocamere e supporterà sia la fotocamera mono che quella stereo. Ad esempio, il sistema potrebbe chiedere all'applicazione di eseguire il rendering da una prospettiva alternativa quando l'utente attiva una funzionalità come l' [acquisizione di realtà mista](/hololens/holographic-photos-and-videos) (MRC), a seconda della forma dell'auricolare. Le applicazioni in grado di supportare più fotocamere li ottengono [scegliendo](/uwp/api/Windows.Graphics.Holographic.HolographicViewConfiguration#Windows_Graphics_Holographic_HolographicViewConfiguration) il [tipo](/uwp/api/Windows.Graphics.Holographic.HolographicViewConfigurationKind#Windows_Graphics_Holographic_HolographicViewConfigurationKind) di fotocamere che possono supportare.

## <a name="volume-rendering"></a>Rendering dei volumi

Quando si esegue il rendering di MRIs medicali o di volumi di progettazione in 3D, vengono spesso usate tecniche di [rendering del volume](volume-rendering.md) . Queste tecniche possono essere interessanti in realtà mista, in cui gli utenti possono visualizzare naturalmente tale volume dagli angoli principali, semplicemente spostando l'intestazione.

## <a name="supported-resolutions-on-hololens-first-gen"></a>Risoluzioni supportate su HoloLens (prima generazione)

* La dimensione massima del viewport è una proprietà di [HolographicDisplay](/uwp/api/windows.graphics.holographic.holographicdisplay). Per impostazione predefinita, HoloLens è impostato sulla dimensione massima del viewport, che è 720p (1268x720).
...
* La dimensione minima del viewport supportata in HoloLens (prima generazione) è 50% di 720p, ovvero 360p (634x360). Si tratta di un ViewportScaleFactor di 0,5.
* Non è consigliabile usare elementi inferiori a 540p a causa di una riduzione visiva, ma è possibile usarli per identificare i colli di bottiglia nella velocità di riempimento in pixel.

## <a name="supported-resolutions-on-hololens-2"></a>Risoluzioni supportate su HoloLens 2

* Le dimensioni correnti e massime della destinazione di rendering supportate sono proprietà della [configurazione della vista](/uwp/api/Windows.Graphics.Holographic.HolographicViewConfiguration#Windows_Graphics_Holographic_HolographicViewConfiguration). Per impostazione predefinita, HoloLens 2 è impostato sulla dimensione massima della destinazione di rendering, che è 1440x936.
...
* Le app possono comunque modificare le dimensioni del viewport nello stesso modo in cui sono state effettuate per HoloLens 1. Non esiste una riallocazione di memoria aggiuntiva sulla GPU, quindi può essere modificata a frequenza elevata, ma non può essere usata per ridurre le richieste di memoria sulla GPU.
* Le dimensioni minime del viewport supportate in HoloLens 2 sono 634x412, ViewportScaleFactor di circa 0,44 quando le dimensioni della destinazione di rendering predefinite sono in uso.
