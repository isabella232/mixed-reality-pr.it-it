...
Controllare **Mixed Reality Toolkit Foundation** e fare clic sull'elenco a discesa accanto a esso per selezionare **MRTK 2.7.0,** controllare anche il plug-in **OpenXR di** realtà mista e fare clic sull'elenco a discesa accanto a esso per selezionare la versione più recente disponibile, quindi fare clic sul pulsante Ottieni funzionalità per scaricare i pacchetti selezionati. 
...
Al riavvio di Unity verrà visualizzata la finestra MrTK Project Configurator. In caso contrario, è possibile aprirlo manualmente in **Mixed Reality**  >  **Toolkit**  >  **Utilities**  >  **Configure Project for MRTK**:

![Aprire la finestra del configuratore di progetto MRTK](../images/mr-learning-base/base-02-section5-step1-2-openxr.png)

Fare clic **su Unity OpenXR Plugin (Plug-in Unity OpenXR)** per abilitare la gestione dei plug-in XR e aggiungere i pacchetti necessari al progetto.

![Aggiungere il plug-in OpenXR di Unity ](../images/mr-learning-base/base-02-section5-step1-3-openxr.PNG)

Verranno importati i pacchetti unity necessari per XR Plugin Management, dopo aver fatto clic su **Show XR Plug-In Management Settings** in MRTK project Configurator (Mostra impostazioni di gestione Plug-In XR nel configuratore di progetto MRTK).
...
Nella barra dei menu passare a **Mixed Reality** >  **OpenXR**  >  **Apply recommended project settings for HoloLens 2** per ottenere prestazioni migliori dell'app.

![Finestra Impostazioni progetto 3](../images/mr-learning-base/base-02-section5-step1-8-openxr.PNG)

Usare il menu Unity per aprire MRTK Project Configurator. Nella finestra MRTK Project Configurator fare clic su **Avanti,** quindi fare clic sul **pulsante** Applica per applicare le impostazioni:
...
Al riavvio di Unity aprire MRTK Project Configurator  dal menu unity e fare clic su Avanti e quindi fare clic su Fine **per** completare la configurazione del progetto Unity per OpenXR.
...
Nella finestra Project Settings (Impostazioni progetto) selezionare **XR Plug-in Management** OpenXR (Gestione plug-in XR OpenXR), quindi usare l'elenco a discesa Depth Submission Mode (Modalità di invio profondità) per selezionare  >  Depth **16-bit (Profondità a 16 bit):** 
...
Fare quindi clic su **Avvia** per iniziare a usare Lo strumento per le funzionalità di realtà mista.
...
Selezionare la casella **per Mixed Reality Toolkit Foundation** e fare clic sull'elenco a discesa accanto a  esso per selezionare **MRTK 2.7.0,** quindi fare clic sul pulsante Ottieni funzionalità per scaricare i pacchetti selezionati.
...
Verranno importati i pacchetti unity necessari per la gestione dei plug-in XR, dopo aver fatto clic su **Mostra** impostazioni nel configuratore di progetto MRTK.

![Finestra delle impostazioni del lettore](../images/mr-learning-base/base-02-section5-step1-3xrsdk.PNG)

Verrà visualizzata la finestra Impostazioni progetto **,** nella finestra Impostazioni progetto in Gestione **plug-in XR** Assicurarsi di essere nelle impostazioni di piattaforma UWP (Universal Windows Platform) Anche Assicurarsi che l'opzione Inizializza **XR** all'avvio sia **selezionata** e selezionare Windows Mixed Reality casella di controllo.
...
Nella finestra MRTK Project Configurator  fare clic su Next (Avanti) e quindi usare l'elenco a discesa Audio spatializer (Spazializzatore audio) per selezionare **MS HRTF Spatializer** e quindi fare clic sul pulsante **Apply** (Applica) per applicare l'impostazione:
...
Fare clic **su Next (Avanti)** e quindi su **Done** (Fine) nella finestra MRTK Project Configurator (Configuratore progetto MRTK) per completare la configurazione del progetto Unity per XRSDK.
...
Il primo passaggio consiste nel puntare Mixed  Reality Feature  Tool al percorso del progetto usando il pulsante con i puntini di sospensione, fare clic sul pulsante con i tre puntini di sospensione accanto al percorso del progetto e passare alla cartella del progetto nello strumento di esplorazione, ad esempio _D:\MixedRealityLearning\MRTK Tutorials._
...
Le funzionalità sono raggruppate per categoria per semplificare la ricerca. Fare clic sull'elenco a discesa **mixed reality toolkit** per trovare i pacchetti relativi a Mixed Reality Toolkit.

![Funzionalità di individuazione di MixedRealityFeatureTool](../images/mr-learning-base/base-02-section4-step1-4.PNG)

Selezionare **Mixed Reality Toolkit Foundation** e fare clic sull'elenco a discesa accanto per selezionare **MRTK 2.7.0** e quindi fare clic sul pulsante Get **features** (Ottieni funzionalità) per scaricare i pacchetti selezionati.
...
Nella finestra MRTK Project Configurator (Configuratore progetto MRTK) verificare che tutte le opzioni siano selezionate e usare anche l'elenco a discesa **Spatializer** audio (Spazializzatore audio) per selezionare lo **spatializer MS HRTF** e quindi fare clic sul pulsante **Apply** (Applica) per applicare l'impostazione:
...
Fare clic **su Next (Avanti)** e quindi sul pulsante **Done** (Fine) nella finestra MRTK Project Configurator (Configuratore progetto MRTK) per completare la configurazione del progetto Unity per Legacy XR (XR legacy).
