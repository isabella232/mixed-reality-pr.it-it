...
<span data-ttu-id="3e2f9-127">Il corso illustra anche il caricamento manuale delle immagini, la creazione di tag e il training del servizio per riconoscere oggetti diversi (nell'esempio specificato, una Coppa), impostando la *casella limite* all'interno dell'immagine inviata.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-127">The course will also cover manually uploading images, creating tags, and training the Service, to recognize different objects (in the provided example, a cup), by setting the *Boundary Box* within the image you submit.</span></span> 
...
## <a name="before-you-start"></a><span data-ttu-id="3e2f9-152">Prima di iniziare</span><span class="sxs-lookup"><span data-stu-id="3e2f9-152">Before you start</span></span>
...
7. <span data-ttu-id="3e2f9-235">In questo pannello è necessario verificare alcune impostazioni:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-235">In this panel, a few settings need to be verified:</span></span>
    ...
    1.  <span data-ttu-id="3e2f9-236">Nella scheda **altre impostazioni** :</span><span class="sxs-lookup"><span data-stu-id="3e2f9-236">In the **Other Settings** tab:</span></span>
        ...
        3. <span data-ttu-id="3e2f9-239">Il **livello di compatibilità API** deve essere **.NET 4,6**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-239">**API Compatibility Level** should be **.NET 4.6**.</span></span>
...
<span data-ttu-id="3e2f9-290">Per creare questa classe:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-290">To create this class:</span></span>
...
4.  <span data-ttu-id="3e2f9-329">Aggiungere quindi le variabili seguenti all'interno della classe **SpatialMapping** , sopra il metodo **Start ()** :</span><span class="sxs-lookup"><span data-stu-id="3e2f9-329">Then, add the following variables inside the **SpatialMapping** class, above the **Start()** method:</span></span>
...
6.  <span data-ttu-id="3e2f9-331">Eliminare il metodo **Update ()** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-331">Delete the **Update()** method.</span></span>
...
7. <span data-ttu-id="3e2f9-344">Assicurarsi di salvare le modifiche in **Visual Studio** prima di tornare a **Unity**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-344">Be sure to save your changes in **Visual Studio**, before returning to **Unity**.</span></span>
...
8.  <span data-ttu-id="3e2f9-360">Aggiungere il metodo **FinaliseLabel ()** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-360">Add the **FinaliseLabel()** method.</span></span> <span data-ttu-id="3e2f9-361">È responsabile di:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-361">It is responsible for:</span></span>
...
10. <span data-ttu-id="3e2f9-367">Assicurarsi di salvare le modifiche in **Visual Studio** prima di tornare a **Unity**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-367">Be sure to save your changes in **Visual Studio**, before returning to **Unity**.</span></span>

    > [!IMPORTANT]
    > <span data-ttu-id="3e2f9-368">Prima di continuare, aprire la classe **CustomVisionAnalyser** e, all'interno del metodo **AnalyseLastImageCaptured ()** , *rimuovere il commento* dalle righe seguenti:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-368">Before you continue, open the **CustomVisionAnalyser** class, and within the **AnalyseLastImageCaptured()** method, *uncomment* the following lines:</span></span>
    >
    >   ```csharp
    >   // Create a texture. Texture size does not matter, since 
    >   // LoadImage will replace with the incoming image size.
...
> [!NOTE]
> <span data-ttu-id="3e2f9-369">Non preoccuparsi del messaggio "Impossibile trovare la classe **ImageCapture** ", che verrà creata nel capitolo successivo.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-369">Do not worry about the **ImageCapture** class 'could not be found' message, you will create it in the next chapter.</span></span>


## <a name="chapter-10---create-the-imagecapture-class"></a><span data-ttu-id="3e2f9-370">Capitolo 10: creare la classe ImageCapture</span><span class="sxs-lookup"><span data-stu-id="3e2f9-370">Chapter 10 - Create the ImageCapture class</span></span>

<span data-ttu-id="3e2f9-371">La classe successiva che si intende creare è la classe **ImageCapture** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-371">The next class you are going to create is the **ImageCapture** class.</span></span>

<span data-ttu-id="3e2f9-372">Questa classe è responsabile di:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-372">This class is responsible for:</span></span>

*   <span data-ttu-id="3e2f9-373">Acquisizione di un'immagine con la fotocamera HoloLens e relativa archiviazione nella cartella dell' *app* .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-373">Capturing an image using the HoloLens camera and storing it in the *App* folder.</span></span>
*   <span data-ttu-id="3e2f9-374">Gestione dei movimenti *Tap* dall'utente.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-374">Handling *Tap* gestures from the user.</span></span>

<span data-ttu-id="3e2f9-375">Per creare questa classe:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-375">To create this class:</span></span>

1.  <span data-ttu-id="3e2f9-376">Passare alla cartella **Scripts** creata in precedenza.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-376">Go to the **Scripts** folder you created previously.</span></span>

2.  <span data-ttu-id="3e2f9-377">Fare clic con il pulsante destro del mouse all'interno della cartella, quindi scegliere **Crea**  >  **\# script C**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-377">Right-click inside the folder, then click **Create** > **C\# Script**.</span></span> <span data-ttu-id="3e2f9-378">Denominare lo script **ImageCapture**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-378">Name the script **ImageCapture**.</span></span>

3.  <span data-ttu-id="3e2f9-379">Fare doppio clic sul nuovo script **ImageCapture** per aprirlo con **Visual Studio.**</span><span class="sxs-lookup"><span data-stu-id="3e2f9-379">Double-click on the new **ImageCapture** script to open it with **Visual Studio.**</span></span>

4.  <span data-ttu-id="3e2f9-380">Sostituire gli spazi dei nomi all'inizio del file con il codice seguente:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-380">Replace the namespaces at the top of the file with the following:</span></span>
...
5.  <span data-ttu-id="3e2f9-381">Aggiungere quindi le variabili seguenti all'interno della classe **ImageCapture** , sopra il metodo **Start ()** :</span><span class="sxs-lookup"><span data-stu-id="3e2f9-381">Then add the following variables inside the **ImageCapture** class, above the **Start()** method:</span></span>
...
6.  <span data-ttu-id="3e2f9-382">È ora necessario aggiungere il codice per i metodi **svegli ()** e **Start ()** :</span><span class="sxs-lookup"><span data-stu-id="3e2f9-382">Code for **Awake()** and **Start()** methods now needs to be added:</span></span>
...
7.  <span data-ttu-id="3e2f9-383">Implementare un gestore che verrà chiamato quando si verifica un movimento TAP:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-383">Implement a handler that will be called when a Tap gesture occurs:</span></span>
    ...
    > [!IMPORTANT]
    > <span data-ttu-id="3e2f9-384">Quando il cursore è **verde**, significa che la fotocamera è disponibile per l'immagine.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-384">When the cursor is **green**, it means the camera is available to take the image.</span></span> <span data-ttu-id="3e2f9-385">Quando il cursore è **rosso**, significa che la fotocamera è occupata.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-385">When the cursor is **red**, it means the camera is busy.</span></span>

8.  <span data-ttu-id="3e2f9-386">Aggiungere il metodo usato dall'applicazione per avviare il processo di acquisizione dell'immagine e archiviare l'immagine:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-386">Add the method that the application uses to start the image capture process and store the image:</span></span>
...
9.  <span data-ttu-id="3e2f9-387">Aggiungere i gestori che verranno chiamati quando la foto è stata acquisita e quando è pronta per essere analizzata.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-387">Add the handlers that will be called when the photo has been captured and for when it is ready to be analyzed.</span></span> <span data-ttu-id="3e2f9-388">Il risultato viene quindi passato a **CustomVisionAnalyser** per l'analisi.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-388">The result is then passed to the **CustomVisionAnalyser** for analysis.</span></span>
...
10. <span data-ttu-id="3e2f9-389">Assicurarsi di salvare le modifiche in **Visual Studio** prima di tornare a **Unity**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-389">Be sure to save your changes in **Visual Studio**, before returning to **Unity**.</span></span>

## <a name="chapter-11---setting-up-the-scripts-in-the-scene"></a><span data-ttu-id="3e2f9-390">Capitolo 11-Configurazione degli script nella scena</span><span class="sxs-lookup"><span data-stu-id="3e2f9-390">Chapter 11 - Setting up the scripts in the scene</span></span>

<span data-ttu-id="3e2f9-391">Ora che è stato scritto tutto il codice necessario per questo progetto, è il momento di configurare gli script nella scena e sui prefabbricati per comportarsi correttamente.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-391">Now that you have written all of the code necessary for this project, is time to set up the scripts in the scene, and on the prefabs, for them to behave correctly.</span></span>

1.  <span data-ttu-id="3e2f9-392">Nell' **editor di Unity** selezionare la **fotocamera principale** nel **Pannello gerarchia**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-392">Within the **Unity Editor**, in the **Hierarchy Panel**, select the **Main Camera**.</span></span>
2.  <span data-ttu-id="3e2f9-393">Nel **pannello Inspector**, con la **fotocamera principale** selezionata, fare clic su **Add Component**, quindi cercare lo script **SceneOrganiser** e fare doppio clic su per aggiungerlo.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-393">In the **Inspector Panel**, with the **Main Camera** selected, click on **Add Component**, then search for **SceneOrganiser** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-38.png)

3.  <span data-ttu-id="3e2f9-394">Nel **Pannello del progetto** aprire la **cartella prefabbricati**, trascinare l' **etichetta** prefabbricato nell'area di input di destinazione di riferimento vuota dell' *etichetta* , nello script **SceneOrganiser** appena aggiunto alla *fotocamera principale*, come illustrato nell'immagine seguente:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-394">In the **Project Panel**, open the **Prefabs folder**, drag the **Label** prefab into the *Label* empty reference target input area, in the **SceneOrganiser** script that you have just added to the *Main Camera*, as shown in the image below:</span></span>

    ![](images/AzureLabs-Lab310-39.png)

4.  <span data-ttu-id="3e2f9-395">Nel **Pannello gerarchia** selezionare l'elemento figlio **GazeCursor** della **fotocamera principale**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-395">In the **Hierarchy Panel**, select the **GazeCursor** child of the **Main Camera**.</span></span>
5.  <span data-ttu-id="3e2f9-396">Nel **pannello Inspector**, con **GazeCursor** selezionato, fare clic su **Add Component**, quindi cercare **GazeCursor** script e fare doppio clic su per aggiungerlo.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-396">In the **Inspector Panel**, with the **GazeCursor** selected, click on **Add Component**, then search for **GazeCursor** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-40.png)

6.  <span data-ttu-id="3e2f9-397">Anche in questo caso, nel **Pannello gerarchia** selezionare l'elemento figlio **SpatialMapping** della **fotocamera principale**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-397">Again, in the **Hierarchy Panel**, select the **SpatialMapping** child of the **Main Camera**.</span></span>
7.  <span data-ttu-id="3e2f9-398">Nel **pannello Inspector**, con **SpatialMapping** selezionato, fare clic su **Add Component**, quindi cercare **SpatialMapping** script e fare doppio clic su per aggiungerlo.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-398">In the **Inspector Panel**, with the **SpatialMapping** selected, click on **Add Component**, then search for **SpatialMapping** script and double-click, to add it.</span></span>

    ![](images/AzureLabs-Lab310-41.png)

<span data-ttu-id="3e2f9-399">Gli script rimanenti non impostati verranno aggiunti dal codice nello script **SceneOrganiser** , durante la fase di esecuzione.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-399">The remaining scripts thats you have not set will be added by the code in the **SceneOrganiser** script, during runtime.</span></span>

## <a name="chapter-12---before-building"></a><span data-ttu-id="3e2f9-400">Capitolo 12-prima della compilazione</span><span class="sxs-lookup"><span data-stu-id="3e2f9-400">Chapter 12 - Before building</span></span>

<span data-ttu-id="3e2f9-401">Per eseguire un test completo dell'applicazione, è necessario sideload in Microsoft HoloLens.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-401">To perform a thorough test of your application you will need to sideload it onto your Microsoft HoloLens.</span></span>

<span data-ttu-id="3e2f9-402">Prima di procedere, verificare quanto segue:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-402">Before you do, ensure that:</span></span>

-  <span data-ttu-id="3e2f9-403">Tutte le impostazioni indicate nel [capitolo 3](#chapter-3---set-up-the-unity-project) sono impostate correttamente.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-403">All the settings mentioned in the [Chapter 3](#chapter-3---set-up-the-unity-project) are set correctly.</span></span>
- <span data-ttu-id="3e2f9-404">Lo script **SceneOrganiser** è associato all'oggetto **principale della fotocamera** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-404">The script **SceneOrganiser** is attached to the **Main Camera** object.</span></span>
- <span data-ttu-id="3e2f9-405">Lo script **GazeCursor** è associato all'oggetto **GazeCursor** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-405">The script **GazeCursor** is attached to the **GazeCursor** object.</span></span>
- <span data-ttu-id="3e2f9-406">Lo script **SpatialMapping** è associato all'oggetto **SpatialMapping** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-406">The script **SpatialMapping** is attached to the **SpatialMapping** object.</span></span>
- <span data-ttu-id="3e2f9-407">Nel [capitolo 5](#chapter-5---create-the-customvisionanalyser-class), passaggio 6:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-407">In [Chapter 5](#chapter-5---create-the-customvisionanalyser-class), Step 6:</span></span>

    - <span data-ttu-id="3e2f9-408">Assicurarsi di inserire la **chiave di stima del servizio** nella variabile **predictionKey** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-408">Make sure you insert your **Service Prediction Key** into the **predictionKey** variable.</span></span>
    - <span data-ttu-id="3e2f9-409">L' **endpoint di stima** è stato inserito nella classe **predictionEndpoint** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-409">You have inserted your **Prediction Endpoint** into the **predictionEndpoint** class.</span></span>

## <a name="chapter-13---build-the-uwp-solution-and-sideload-your-application"></a><span data-ttu-id="3e2f9-410">Capitolo 13: compilare la soluzione UWP e sideload l'applicazione</span><span class="sxs-lookup"><span data-stu-id="3e2f9-410">Chapter 13 - Build the UWP solution and sideload your application</span></span>

<span data-ttu-id="3e2f9-411">A questo punto si è pronti per compilare l'applicazione come soluzione UWP che sarà possibile distribuire in Microsoft HoloLens.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-411">You are now ready to build you application as a UWP Solution that you will be able to deploy on to the Microsoft HoloLens.</span></span> <span data-ttu-id="3e2f9-412">Per avviare il processo di compilazione:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-412">To begin the build process:</span></span>

1.  <span data-ttu-id="3e2f9-413">Passare a **File > impostazioni di compilazione**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-413">Go to **File > Build Settings**.</span></span>

2.  <span data-ttu-id="3e2f9-414">Seleziona i **\# progetti Unity C**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-414">Tick **Unity C\# Projects**.</span></span>

3.  <span data-ttu-id="3e2f9-415">Fare clic su **Aggiungi scene aperte**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-415">Click on **Add Open Scenes**.</span></span> <span data-ttu-id="3e2f9-416">Verrà aggiunta la scena attualmente aperta alla compilazione.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-416">This will add the currently open scene to the build.</span></span>

    ![](images/AzureLabs-Lab310-42.png)

4.  <span data-ttu-id="3e2f9-417">Fare clic su **Compila**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-417">Click **Build**.</span></span> <span data-ttu-id="3e2f9-418">Unity avvierà una finestra di *Esplora file* , in cui è necessario creare e quindi selezionare una cartella in cui compilare l'app.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-418">Unity will launch a *File Explorer* window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="3e2f9-419">Creare la cartella adesso e denominarla **app**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-419">Create that folder now, and name it **App**.</span></span> <span data-ttu-id="3e2f9-420">Quindi, con la cartella dell' **app** selezionata, fare clic su **Seleziona cartella**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-420">Then with the **App** folder selected, click **Select Folder**.</span></span>

5.  <span data-ttu-id="3e2f9-421">Unity inizierà a compilare il progetto nella cartella dell' **app** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-421">Unity will begin building your project to the **App** folder.</span></span>

6.  <span data-ttu-id="3e2f9-422">Una volta completata la compilazione di Unity (potrebbe richiedere del tempo), verrà aperta una finestra **Esplora file** nella posizione della compilazione (controllare la barra delle applicazioni, perché potrebbe non essere sempre visualizzata sopra le finestre, ma verrà inviata una notifica sull'aggiunta di una nuova finestra).</span><span class="sxs-lookup"><span data-stu-id="3e2f9-422">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

7.  <span data-ttu-id="3e2f9-423">Per eseguire la distribuzione in Microsoft HoloLens, è necessario l'indirizzo IP del dispositivo (per la distribuzione remota) e per assicurarsi che abbia anche la **modalità di sviluppo** impostata.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-423">To deploy on to Microsoft HoloLens, you will need the IP Address of that device (for Remote Deploy), and to ensure that it also has **Developer Mode** set.</span></span> <span data-ttu-id="3e2f9-424">Per eseguire questa operazione:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-424">To do this:</span></span>

    1.  <span data-ttu-id="3e2f9-425">Quando si indossa il HoloLens, aprire le **Impostazioni**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-425">Whilst wearing your HoloLens, open the **Settings**.</span></span>

    2.  <span data-ttu-id="3e2f9-426">Passa a **rete &**  >  **Wi-Fi**  >  **Opzioni avanzate** Wi-Fi Internet</span><span class="sxs-lookup"><span data-stu-id="3e2f9-426">Go to **Network & Internet** > **Wi-Fi** > **Advanced Options**</span></span>

    3.  <span data-ttu-id="3e2f9-427">Prendere nota dell'indirizzo **IPv4** .</span><span class="sxs-lookup"><span data-stu-id="3e2f9-427">Note the **IPv4** address.</span></span>

    4.  <span data-ttu-id="3e2f9-428">Tornare quindi a Settings ( **Impostazioni**) e quindi **aggiornare & Security**  >  **per gli sviluppatori**</span><span class="sxs-lookup"><span data-stu-id="3e2f9-428">Next, navigate back to **Settings**, and then to **Update & Security** > **For Developers**</span></span>

    5.  <span data-ttu-id="3e2f9-429">Impostare la **modalità di sviluppo** *su*.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-429">Set **Developer Mode** *On*.</span></span>

8.  <span data-ttu-id="3e2f9-430">Passare alla nuova compilazione Unity (cartella **app** ) e aprire il file della soluzione con **Visual Studio**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-430">Navigate to your new Unity build (the **App** folder) and open the solution file with **Visual Studio**.</span></span>

9.  <span data-ttu-id="3e2f9-431">Nella configurazione della soluzione selezionare **debug**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-431">In the Solution Configuration select **Debug**.</span></span>

10. <span data-ttu-id="3e2f9-432">Nella piattaforma soluzione selezionare **x86, computer remoto**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-432">In the Solution Platform, select **x86, Remote Machine**.</span></span> <span data-ttu-id="3e2f9-433">Verrà richiesto di inserire l' **indirizzo IP** di un dispositivo remoto (Microsoft HoloLens, in questo caso, annotato).</span><span class="sxs-lookup"><span data-stu-id="3e2f9-433">You will be prompted to insert the **IP address** of a remote device (the Microsoft HoloLens, in this case, which you noted).</span></span>

    ![](images/AzureLabs-Lab310-43.png)

11. <span data-ttu-id="3e2f9-434">Passare al menu **Compila** e fare clic su **Distribuisci soluzione** per sideload l'applicazione in HoloLens.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-434">Go to the **Build** menu and click on **Deploy Solution** to sideload the application to your HoloLens.</span></span>

12. <span data-ttu-id="3e2f9-435">L'app verrà ora visualizzata nell'elenco delle app installate in Microsoft HoloLens, pronto per l'avvio.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-435">Your app should now appear in the list of installed apps on your Microsoft HoloLens, ready to be launched!</span></span>

### <a name="to-use-the-application"></a><span data-ttu-id="3e2f9-436">Per utilizzare l'applicazione:</span><span class="sxs-lookup"><span data-stu-id="3e2f9-436">To use the application:</span></span>

* <span data-ttu-id="3e2f9-437">Esaminare un oggetto, che è stato sottoposto a training con la **servizio visione artificiale personalizzato di Azure, il rilevamento degli oggetti** e usare il **gesto Tap**.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-437">Look at an object, which you have trained with your **Azure Custom Vision Service, Object Detection**, and use the **Tap gesture**.</span></span>
* <span data-ttu-id="3e2f9-438">Se l'oggetto viene rilevato correttamente, verrà visualizzato un *testo dell'etichetta* con spazio globale con il nome del tag.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-438">If the object is successfully detected, a world-space *Label Text* will appear with the tag name.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="3e2f9-439">Ogni volta che si acquisisce una foto e la si invia al servizio, è possibile tornare alla pagina del servizio e ripetere il training del servizio con le immagini appena acquisite.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-439">Every time you capture a photo and send it to the Service, you can go back to the Service page and retrain the Service with the newly captured images.</span></span> <span data-ttu-id="3e2f9-440">All'inizio, probabilmente sarà anche necessario correggere i *riquadri delimitativi* in modo da essere più accurati e ripetere il training del servizio.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-440">At the beginning, you will probably also have to correct the *Bounding Boxes* to be more accurate and retrain the Service.</span></span>

> [!NOTE]
> <span data-ttu-id="3e2f9-441">Il testo dell'etichetta inserito potrebbe non essere visualizzato vicino all'oggetto quando i sensori Microsoft HoloLens e/o SpatialTrackingComponent in Unity non riescono a collocare i Collider appropriati, relativi agli oggetti reali.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-441">The Label Text placed might not appear near the object when the Microsoft HoloLens sensors and/or the SpatialTrackingComponent in Unity fails to place the appropriate colliders, relative to the real world objects.</span></span> <span data-ttu-id="3e2f9-442">Se questo è il caso, provare a usare l'applicazione in un'area diversa.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-442">Try to use the application on a different surface if that is the case.</span></span>

## <a name="your-custom-vision-object-detection-application"></a><span data-ttu-id="3e2f9-443">Applicazione di rilevamento oggetti Visione personalizzata</span><span class="sxs-lookup"><span data-stu-id="3e2f9-443">Your Custom Vision, Object Detection application</span></span>

<span data-ttu-id="3e2f9-444">Congratulazioni, è stata creata un'app per realtà mista che sfrutta il Visione personalizzata di Azure, l'API di rilevamento oggetti, che può riconoscere un oggetto da un'immagine, quindi fornire una posizione approssimativa per tale oggetto nello spazio 3D.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-444">Congratulations, you built a mixed reality app that leverages the Azure Custom Vision, Object Detection API, which can recognize an object from an image, and then provide an approximate position for that object in 3D space.</span></span>

![](images/AzureLabs-Lab310-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="3e2f9-445">Esercizi aggiuntivi</span><span class="sxs-lookup"><span data-stu-id="3e2f9-445">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="3e2f9-446">Esercizio 1</span><span class="sxs-lookup"><span data-stu-id="3e2f9-446">Exercise 1</span></span>

<span data-ttu-id="3e2f9-447">Aggiungendo all'etichetta di testo, utilizzare un cubo semi-trasparente per eseguire il wrapping dell'oggetto reale in un rettangolo di *delimitazione* 3D.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-447">Adding to the Text Label, use a semi-transparent cube to wrap the real object in a 3D *Bounding Box*.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="3e2f9-448">Esercizio 2</span><span class="sxs-lookup"><span data-stu-id="3e2f9-448">Exercise 2</span></span>

<span data-ttu-id="3e2f9-449">Eseguire il training del Servizio visione artificiale personalizzato per riconoscere più oggetti.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-449">Train your Custom Vision Service to recognize more objects.</span></span>

### <a name="exercise-3"></a><span data-ttu-id="3e2f9-450">Esercizio 3</span><span class="sxs-lookup"><span data-stu-id="3e2f9-450">Exercise 3</span></span>

<span data-ttu-id="3e2f9-451">Riprodurre un suono quando viene riconosciuto un oggetto.</span><span class="sxs-lookup"><span data-stu-id="3e2f9-451">Play a sound when an object is recognized.</span></span>

### <a name="exercise-4"></a><span data-ttu-id="3e2f9-452">Esercizio 4</span><span class="sxs-lookup"><span data-stu-id="3e2f9-452">Exercise 4</span></span>

<span data-ttu-id="3e2f9-453">Usare l'API per eseguire nuovamente il training del servizio con le stesse immagini analizzate dall'app, in modo da rendere più accurato il servizio (eseguire contemporaneamente la stima e la formazione).</span><span class="sxs-lookup"><span data-stu-id="3e2f9-453">Use the API to re-train your Service with the same images your app is analyzing, so to make the Service more accurate (do both prediction and training simultaneously).</span></span>
