...
Gli auricolari per la realtà mista di Windows non supportano ufficialmente l'uso di cavi di estensione USB, HDMI o DisplayPort. Se si usano questi cavi, l'esperienza di realtà mista potrebbe influire a causa delle varianze nell'integrità del segnale e nell'alimentazione del bus tra il controller USB del PC e l'auricolare della realtà mista. Provare a usare l'auricolare senza cavi di estensione se:
* La visualizzazione dell'auricolare Mostra brevemente una schermata blu, quindi viene riavviato o completamente deenumerato durante l'utilizzo
* L'audio della cuffia si interrompe o diventa glitchy
* Scorrimenti rapidi dell'auricolare tra il nero e la visualizzazione corretta
...
* Se si usano schede per connettere l'auricolare al PC, verificare che supportino la realtà mista di Windows e che siano in grado di supportare 4K. Provare anche a connettere l'adattatore al PC prima di connettere l'auricolare alla scheda.
...
* Se il PC dispone di grafica sia integrata che discreta, assicurarsi di usare la porta HDMI o DisplayPort sulla scheda grafica attiva. Questo potrebbe significare che è necessario connettere la visualizzazione del PC a una porta non HDMI.
* Se il PC dispone di grafica sia integrata che discreta e i grafici integrati sono meno recenti e non supportano la realtà mista di Windows, provare a disabilitare la GPU integrata.
* Connettere un monitor PC alla porta HDMI o DisplayPort del computer. Assicurarsi che i driver grafici siano aggiornati. Scaricare e installare i driver direttamente da AMD, NVIDIA o Intel, perché saranno probabilmente più recenti rispetto a quelli pubblicati per Windows Update.
...
Quando si usa l'auricolare, la realtà mista di Windows potrebbe richiedere alcuni secondi per ricaricare lo spazio. Se il messaggio non viene rimosso, assicurarsi che l'adesivo protettivo sia stato rimosso dal sensore di prossimità all'interno dell'auricolare tra le lenti. Se il problema persiste, contattare il produttore dell'auricolare.
...
- Assicurarsi che i cavi USB e HDMI o DisplayPort dell'auricolare siano connessi alle porte corrette nel PC. Ecco come identificare le porte corrette:
    ...
    - Se il computer dispone di due porte HDMI o DisplayPort, utilizzare quello che si connette alla scheda grafica, non alla scheda madre del computer. Questa operazione non è sempre ovvia, anche se le porte discrete si trovano spesso in uno slot di espansione nel computer. Se si prova a usare una porta e non funziona, provare l'altra.

- Scollegare i cavi USB e HDMI o DisplayPort dall'auricolare per assicurarsi che siano connessi in modo sicuro. Quando si collega il cavo USB, provare a non sospendere l'inserimento del cavo USB.
- Se viene visualizzato l'enumerazione parziale dell'auricolare, provare a usare un hub USB 3,0 alimentato esternamente, ad esempio, una serie di dispositivi USB enumerano, ma niente sotto "cuffie di realtà miste" in Device Manager.
...
Nota: se si dispone di una versione N di Windows, è necessario passare a un'edizione regolare di Windows 10 per usare la realtà mista di Windows.